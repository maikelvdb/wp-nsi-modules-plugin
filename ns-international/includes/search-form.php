...
<div class="ns-international_searchform ns-from" style="--nsi-max-width: <?= $maxWidth; ?>px; --nsi-margin-bottom: <?= $marginBottom; ?>px;">
	<form action="#" methode="get" class="form-container">
		<div class="ns-stations-group">
			<?= renderNsStationsSelect(TextValues::get("from"), "from", $a['from'], $id . '_from'); ?>
			<div class="ns-form-switch">
			...
			<?= renderNsStationsSelect(TextValues::get("to"), "to", $a['to'], $id . '_to'); ?>
		</div>
		<div class="content-none">
			<?= render_input(TextValues::get("date"), "text", "date", date('d-m-Y', strtotime(date('Y-m-d') . ' +1 day')), "js-date", ["min" => date('Y-m-d')]); ?>
		</div>
		<div class="button-wrapper"><?= render_button(TextValues::get("search"), "nsi-cta"); ?></div>
    </form>
</div>
